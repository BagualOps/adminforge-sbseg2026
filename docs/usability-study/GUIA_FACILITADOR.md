...
sucesso e dicas por tarefa, e o fechamento (rubrica de Whitten & Tygar, TAM,
entrevista). Para a escolha do método, ver [`METODOLOGIA-OPCOES.md`](METODOLOGIA-OPCOES.md)
...
- Pré-questionário de perfil (Parte A de [`QUESTIONARIO.md`](QUESTIONARIO.md)): cargo, anos
  administrando Linux, com que ferramentas gerencia acesso hoje, familiaridade com CLI.
...
**nº de `--help`/`-h`** e **erro perigoso** (sim/não + descrição verbatim do que a pessoa disse).
...
> **Versão Google Forms.** O script [`criar-formulario-facilitador.gs`](criar-formulario-facilitador.gs)
> gera esta folha como um Google Form (um por participante): cabeçalho, métricas das Tarefas 1–9,
> episódios de dificuldade, rubrica de Whitten & Tygar, notas da entrevista e observações finais.
> O modelo em texto abaixo continua válido para uso em papel.

```
Participante: P__    Data: ____/____/____    Facilitador: ____________

Tarefa | Resultado          | Tempo | #help | Erro perigoso? | Notas
-------+--------------------+-------+-------+----------------+---------------------------
  0    | (aquecimento)      |   -   |   -   |       -        |
...
Rubrica Whitten & Tygar (sim / parcial / não + 1 frase de evidência):
  (1) foi informada de forma confiável das tarefas de segurança que precisava realizar?  ____  - 
  (2) conseguiu descobrir como executá-las com sucesso?                                   ____  - 
  (3) NÃO cometeu erros perigosos?                                    ____  - 
  (4) ficou confortável com a interface para continuar usando?            ____  - 
...
1. **Rubrica de Whitten & Tygar** - preencher na folha de observação as 4 propriedades (`sim`/`parcial`/
   `não` + 1 frase de evidência): a pessoa (1) foi informada de forma confiável das tarefas de segurança
   que precisava realizar; (2) conseguiu descobrir como executá-las com sucesso; (3) **não cometeu erros
   perigosos**; (4) ficou suficientemente confortável com a interface para continuar usando.
2. **Questionário pós-teste** - Parte C de [`QUESTIONARIO.md`](QUESTIONARIO.md): **TAM**
   (Utilidade Percebida, Facilidade de Uso Percebida, Intenção de Uso). A pontuação está na Parte E.
3. **Entrevista semiestruturada (5–10 min)** - roteiro na Parte D de
   [`QUESTIONARIO.md`](QUESTIONARIO.md); registrar respostas/citações:
